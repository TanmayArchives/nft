...
export function UserFeed({
  ...
  const handleFollowStatusChange = async (username: string, newStatus: string) => {
        ...
        {users.map((user: UserProfile) => (
          ...
          >
            ...
            <div className="flex transition-all p-4 group-hover:backdrop-blur-sm rounded-t-2xl duration-500 ease-in-out flex-col group-hover:bottom-0 absolute bottom-[-3.5rem] left-[50%] translate-x-[-50%] z-[20] w-full gap-6">
              <div className="flex text-foreground items-center justify-between w-full">
                ...
                <div className="flex-shrink-0">
                  <FollowButton
                    ...
                    onFollowChange={(newStatus) => handleFollowStatusChange(user.username, newStatus)}
                  />
                </div>
              </div>
              {user.isPrivate ? (
